...
impl SpringsTestbed {
    pub fn new(config: &testbed::Config) -> SpringsTestbed {
        ...
        let distance = (box_body.transform.world_pos(box_anchor)
            - circle_body.transform.world_pos(circle_anchor))
        .length();
        ...
        world.add_joint(
            (box_id, circle_id),
            SpringJoint::new(box_anchor, circle_anchor, distance, 1.5, 0.7).into_joint(),
        );
...
impl testbed::Testbed for SpringsTestbed {
    fn sfml_loop(&mut self, input: &testbed::Input, dt: f32) {
        ...
        if input.right_mouse_released {
            let Joint::Spring(ref mut joint) = &mut self
                .world
                .get_joints_mut((self.box_id, self.circle_id))
                .unwrap()[0];
            joint.frequency += 1.0;
        }
    ...
    fn sfml_draw(&mut self, canvas: &mut testbed::Canvas, dt: f32) {
        ...
        let Joint::Spring(ref joint) = &self
            .world
            .get_joints((self.box_id, self.circle_id))
            .unwrap()[0];
        {
