...
impl StacksTestbed {
    pub fn new(config: &testbed::Config) -> StacksTestbed {
        ...
        //  Stacks
        let mut x = Vec2::new(
            -window_width / 2.0 + SIDE * 0.5 + 3.5,
            -window_height / 2.0 + SIDE * 0.5 + ground_height,
        );
        ...
        for i in 0..N {
            ...
            for _ in i..N {
                let mut body =
                    Body::new(square.clone().into_shape(), 10.0, Material::new(0.3, 0.3));
...
impl testbed::Testbed for StacksTestbed {
    ...
    fn sfml_draw(&mut self, canvas: &mut testbed::Canvas, dt: f32) {
        ...
        canvas.draw_text(
            format!("Velocity iterations: {}", self.world.velocity_iterations),
            16,
        );
        canvas.draw_text(
            format!("Position iterations: {}", self.world.position_iterations),
            16,
        );
