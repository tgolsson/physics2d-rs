...
use crate::{
    collision::broad_phase::{BoundsTreeBroadPhase, BroadPhase},
    collision::{collide, ContactConstraint},
    joint::Joint,
};
use collections::ConstraintSolverMap;
...
impl World {
    ...
    pub fn add_joint(&mut self, bodies: (BodyId, BodyId), joint: Joint) {
        ...
        let body_joints = self.joints.entry(bodies).or_insert_with(Vec::new);
    ...
    pub fn update(&mut self, dt: f32) {
...

        {
            ...
            self.contact_constraints
                .retain(|pair, _| pair.with(bodies, |a, b| a.bounds.intersects(&b.bounds)));
        }

        self.broad_phase
            .new_potential_pairs(&self.bodies, &mut self.contact_constraints);
...

        {
            ...
            self.contact_constraints.retain(|pair, constraints| {
                ...
                if let Some(new_contacts) = collide(body_a, body_b) {
                    let new_constraints = if !constraints.is_empty() {
        ...
        self.contact_constraints
            .initialize_velocity(&self.bodies, dt);
        self.contact_constraints
            .warm_start_velocity(&mut self.bodies, dt);

        self.joints.initialize_velocity(&self.bodies, dt);
        ...
        for _ in 0..self.velocity_iterations {
            ...
            self.contact_constraints
                .solve_velocity(&mut self.bodies, dt);
        }
