...
impl Transform {
    ...
    ///
    /// assert!((t.world_dir(Vec2::RIGHT) - Vec2::UP).length() < 1e-07);
    /// ```
    ...
    /// assert!((t.local_pos(Vec2::new(1.0, 3.0)) - Vec2::new(1.0, 0.0)).length() < 1e-07);
    /// ```
    ...
    /// let t = Transform::new(Vec2::new(1000.01, 333.333), math::PI / 2.0);
    ///
    /// assert!((t.local_dir(Vec2::UP) - Vec2::RIGHT).length() < 1e-07);
    /// ```
