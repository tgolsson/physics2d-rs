...
use crate::util::pool;
use crate::world::{Bodies, Body, BodyId, BodyPair, ConstraintsMap};
...
#[derive(Clone)]
struct BoundsTree<T: Default + Clone> {
    pool: pool::Pool<Node<T>>,
    root_id: NodeId,
}

impl<T: Default + Clone> BoundsTree<T> {
    ...
    fn balance_subtree(&mut self, node_id: NodeId) -> NodeId {
        ...
        let height_diff =
            self.get_node(right_id).height as i32 - self.get_node(left_id).height as i32;
    ...
    fn insert_leaf(&mut self, bounds: Bounds, data: T) -> NodeId {
        ...
        while !self.get_node(sibling_id).is_leaf() {
            ...
            let cost_right =
                right.bounds.union(&leaf_bounds).perimeter() - right.bounds.perimeter();
...
impl BroadPhase for BoundsTreeBroadPhase {
    ...
    fn create_proxy(&mut self, body: &Body) -> ProxyId {
        self.tree
            .insert_leaf(body.bounds.expand_by(EXPANSION_FACTOR), body.id)
    }
