...
pub fn collide(a: &Body, b: &Body) -> Option<Vec<Contact>> {
    match b.shape {
        Shape::Circle(ref circle) => match_fn_to_shape!(&a.shape, collide(a, circle, b)),
        Shape::Polygon(ref polygon) => match_fn_to_shape!(&a.shape, collide(a, polygon, b)),
    }
}
...
impl Polygon {
    fn face(&self, index: usize) -> Face {
        Face::new(
            self.vertices[index],
            self.vertices[(index + 1) % self.vert_count()],
            self.normals[index],
        )
    }
}
