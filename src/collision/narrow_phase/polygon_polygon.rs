...
impl Collide for Polygon {
    fn collide(
        ...
        let (self_face_idx, self_pen) =
            self.least_penetration_face(self_transform, other, other_transform);

        if self_pen <= 0.0 {
            return None;
        }

        let (other_face_idx, other_pen) =
            other.least_penetration_face(other_transform, self, self_transform);
        ...
        let side_faces_idx = [
            (ref_face_idx + 1) % ref_poly.vert_count(),
        ...
        for side_face_idx in side_faces_idx.iter() {
            ...
            let (clipped, clip_count) = ref_poly
                .face(*side_face_idx)
        ...
        let normal = if self_is_ref_poly {
            ref_face.normal
        } else {
            -ref_face.normal
        };
        ...
        let contacts = inc_points
            .iter()
            .filter_map(|inc_point| {
