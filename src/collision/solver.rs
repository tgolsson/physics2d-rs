...
impl ContactConstraint {
    ...
    pub fn with_contacts(new_contacts: &[Contact]) -> Vec<ContactConstraint> {
        new_contacts
            .iter()
            .map(|contact| ContactConstraint::new(*contact))
            .collect()
    }

    pub fn with_persistent_contacts(
        old_constraints: &[ContactConstraint],
        new_contacts: &[Contact],
    ) -> Vec<ContactConstraint> {
        let mut new_constraints: Vec<ContactConstraint> = new_contacts
            .iter()
            .map(|contact| ContactConstraint::new(*contact))
            .collect();
        ...
        for old_constraint in old_constraints.iter() {
            ...
            if let Some(near_constraint) = new_constraints.iter_mut().find(|c| {
                (c.contact.position - old_constraint.contact.position).sqr_length()
                    <= PERSISTENT_DISTANCE
            }) {
...
impl Constraint for ContactConstraint {
    fn initialize_velocity(&mut self, a: &Body, b: &Body, _dt: f32) {
        ...
        let rel_vel =
            b.velocity - a.velocity + b.angular_vel.cross(&r_b) - a.angular_vel.cross(&r_a);
        ...
        let r_a_tangent_sqr = r_a.sqr_length() - r_a_normal_sqr;

        let r_b_normal = r_b.dot(contact.normal);
        let r_b_normal_sqr = r_b_normal * r_b_normal;
        let r_b_tangent_sqr = r_b.sqr_length() - r_b_normal_sqr;

        let inv_mass_sum = a.inv_mass + b.inv_mass;

        let inv_normal_impulse_factor =
            inv_mass_sum + r_a_tangent_sqr * a.inv_inertia + r_b_tangent_sqr * b.inv_inertia;

        let inv_tangent_impulse_factor =
            inv_mass_sum + r_a_normal_sqr * a.inv_inertia + r_b_normal_sqr * b.inv_inertia;
    ...
    fn solve_velocity(&mut self, a: &mut Body, b: &mut Body, _dt: f32) {
        ...
        let res_bias =
            -self.restitution * f32::max(0.0, rel_vel_normal - RESTITUTION_VELOCITY_SLOP);
        let bias = res_bias;
