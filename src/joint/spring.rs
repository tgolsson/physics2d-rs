...
impl Constraint for SpringJoint {
    fn initialize_velocity(&mut self, a: &Body, b: &Body, dt: f32) {
        ...
        let inv_reduced_mass = inv_mass_sum
            + r_a_normal_perp * r_a_normal_perp * a.inv_inertia
            + r_b_normal_perp * r_b_normal_perp * b.inv_inertia;

        // Initially reduced mass
        self.mass = if inv_reduced_mass != 0.0 {
            1.0 / inv_reduced_mass
        } else {
            0.0
        };
        ...
        self.softness = if inv_softness != 0.0 {
            1.0 / inv_softness
        } else {
            0.0
        };
    ...
    fn solve_velocity(&mut self, a: &mut Body, b: &mut Body, dt: f32) {
        let rel_vel = b.velocity - a.velocity + b.angular_vel.cross(&self.r_b)
            - a.angular_vel.cross(&self.r_a);

        let rel_vel_normal = self.normal.dot(rel_vel);

        // baumgarte bias = beta * x / dt
        let bias =
            self.beta * f32::max(self.x.abs() - SPRING_DISPLACEMENT_SLOP, 0.0) * self.x.signum()
                / dt;
